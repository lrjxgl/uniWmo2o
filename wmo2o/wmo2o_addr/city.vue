<template>
	<view>
		...
		<scroll-view scroll-y="true" class="aaBox" :class="aaBoxClass">
			<view  @click="setCity(item.name)" v-for="(item,index) in searchList" :key="index" class="flex bd-mp-5">
	...
	export default{
		...
		methods:{
			search:function(){
				...
				setTimeout(function(){
					that.app.get({
						url:that.app.apiHost+"/index.php?m=city&a=search&city="+that.keyword,						
			...
			setCity:function(acity){
				this.app.setCity(acity);
				this.city=acity;
				uni.navigateBack();
			},
			setZm:function(zm){
			...
			getPage:function(){
				var that=this;
				that.app.get({
					url:that.app.apiHost+"/index.php?m=city",
					success:function(res){
